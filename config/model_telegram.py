...
class TelegramWord(BaseWord, AddonWordGetter):
    ...
    def export(self) -> str:
        ...
        word_str = f"<b>{self.name}</b>{self._w_affixes}," \
    ...
    def keyboard_cpx(self, show_list: bool = False, slice_start: int = 0):
        return TelegramWordKeyboard(self).keyboard_cpx(show_list=show_list, slice_start=slice_start)
...
class TelegramWordKeyboard:
    ...
    def keyboard_cpx_switcher(self, total_number_of_complexes: int, show: bool = True):
        ...
        _es = f'es {button_text_number}'.strip() if total_number_of_complexes > 1 else ''
    ...
    def keyboard_cpx(self, show_list: bool = False, slice_start: int = 0):
        ...
        if not show_list:
            keyboards = (self.keyboard_cpx_switcher(total_num_of_cpx, show=True), self.kb_cpx_close)
